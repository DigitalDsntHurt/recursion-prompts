...
var powerOfTwo = function(n) {
  var checker = n;
  while (checker > 0) {
    if (checker * checker === n) { return true; }
    checker -= 1;
  }
  return false
};
...
var capitalizeFirst = function(array) {
  ...
  if (array.length === 1) { return [cap(array[0])]; }
  return [cap(array[0])].concat(capitalizeFirst(array.slice(1)));
};
...
var numToText = function(str) {
  ...
  let words = str.split(' ');
  if (words.length === 1) { return digitToString(words[0]); }
  return digitToString(words[0]) + ' ' + numToText(words.slice(1).join(' '));
};
